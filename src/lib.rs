...
use rayon::prelude::*;
use serde_json::{from_slice, from_value, json, to_value, from_str, Serializer, Value};
...
pub fn read_string_as_rtfactory(json: &std::ffi::CStr) -> RTFactory {
	let val = from_str::<Value>(
		&json.to_str().expect("Input string is not valid UTF8")
	)
	.expect("Failed to read JSON string as JSON");

	if val.get("entityTemplates").is_some() {
		convert_2016_factory_to_modern(&from_value(val).expect("Failed to read JSON string as RT struct"))
	} else {
...
#[no_mangle]
pub extern fn convert_entity_to_qn(
	...
	let input_factory_meta : &std::ffi::CStr = unsafe {std::ffi::CStr::from_ptr(factory_meta_json)};
	let input_blueprint : &std::ffi::CStr = unsafe {std::ffi::CStr::from_ptr(blueprint_json)};
	let input_blueprint_meta : &std::ffi::CStr = unsafe {std::ffi::CStr::from_ptr(blueprint_meta_json)};
    ...
    let entity_json = std::ffi::CString::new(to_vec_float_format(&entity)).expect("Error creating Entity JSON CString");
    entity_json.into_raw()
}

#[no_mangle]
pub extern fn free_json_string(json_string: *mut std::os::raw::c_char) {
    unsafe {
        if json_string.is_null() { return }
        std::ffi::CString::from_raw(json_string)
    };
}